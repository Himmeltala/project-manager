import { existsSync, readFileSync, writeFileSync, mkdirSync, readdirSync, promises as fs } from 'fs'
import { join, dirname } from 'path'
import type { Project } from '../../src/types/project'
import { detectType, MARKER_FILES } from './project-type.service'
...
export class ProjectRepository {
  static load(configPath: string): Project[] {
    ...
    try {
      ...
      return raw.map((item: any) => ({
        name: item.name,
        path: item.path,
        projectType: item.type || detectType(item.path),
  ...
  static async discover(rootDir: string): Promise<Project[]> {
    ...
    const absolute = require('path').resolve(rootDir)
  ...
  static merge(existing: Project[], newProjects: Project[]): { merged: Project[]; added: number } {
  ...
  static replace(existing: Project[], newProjects: Project[]): Project[] {
    return [...newProjects]
  }
}
