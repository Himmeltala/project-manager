...
<template>
  <div class="notification-list">
    <div class="notification-header">
      <span class="notification-title">通知{{ store.unreadCount > 0 ? ` (${store.unreadCount})` : '' }}</span>
      <div style="display: flex; gap: 6px">
        <el-button plain size="small" @click="markAllRead">全部已读</el-button>
        <el-button plain size="small" @click="clearAll">清空</el-button>
      </div>
    </div>
    <el-scrollbar class="notification-scroll">
      <div v-if="store.notifications.length === 0" class="empty-hint">
...
.notification-scroll :deep(.el-scrollbar__view) {
  padding: 8px;
}
.notification-item {
  display: flex;
